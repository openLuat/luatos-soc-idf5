...
static void set_lwip_netif(struct netif * lwip_netif, uint8_t adapter_index) {
	if (adapter_index >= NW_ADAPTER_INDEX_LWIP_NETIF_QTY) {
		return; // 超范围了
	}

	net_lwip_ctrl_struct* prvlwip = lwip_ctrls[adapter_index];
	if (NULL == prvlwip) {
		prvlwip = luat_heap_malloc(sizeof(net_lwip_ctrl_struct));
		if (prvlwip == NULL) {
			LLOGE("out of memory when malloc net_lwip_ctrl_struct");
			return;
		}
		memset(prvlwip, 0, sizeof(net_lwip_ctrl_struct));
		net_lwip_init(prvlwip, adapter_index);
		prvlwip->lwip_netif = lwip_netif;
		
		// LLOGD("prvlwip %d %p ok", adapter_index, prvlwip);
		...
		// LLOGD("dns_udp %d %p ok", adapter_index, prvlwip);
		lwip_ctrls[adapter_index] = prvlwip;
	}
}
...
static socket_data_t * net_lwip_create_data_node(net_lwip_ctrl_struct* prvlwip, uint8_t socket_id, uint8_t *data, uint32_t len, luat_ip_addr_t *remote_ip, uint16_t remote_port)
{
...
{
	net_lwip_ctrl_struct* prvlwip = get_lwip_ctrl(adapter_index);
	if (prvlwip == NULL)
		return;
...
static int net_lwip_rx_data(net_lwip_ctrl_struct* prvlwip, int socket_id, struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
	int is_mem_err = 0;
	SOCKET_LOCK(socket_id);
	socket_data_t *data_p = net_lwip_create_data_node(prvlwip, socket_id, NULL, 0, addr, port);
	if (data_p)
	{
...
//		tcp_recved(tpcb, p->tot_len);
		len = p->tot_len;
		if (net_lwip_rx_data(prvlwip, socket_id, p, NULL, 0))
		{
			NET_DBG("no memory!");
			net_lwip_callback_to_nw_task(adapter_index, EV_NW_SOCKET_ERROR, socket_id, 0, 0);
		}
    ...
    const ip_addr_t *addr, u16_t port)
{
	...
	net_lwip_ctrl_struct* prvlwip = get_lwip_ctrl(adapter_index);
	if (prvlwip == NULL) {
		LLOGD("adapter_index %d not found", adapter_index);
		return ERR_OK;
	}
	if (p)
	{
		len = p->tot_len;
		if (net_lwip_rx_data(prvlwip, socket_id, p, addr, port))
		{
			NET_DBG("no memory!");
			net_lwip_callback_to_nw_task(adapter_index, EV_NW_SOCKET_ERROR, socket_id, 0, 0);
		}
...
static void net_lwip_dns_tx_next(net_lwip_ctrl_struct* prvlwip, Buffer_Struct *tx_msg_buf)
{
	int i;
	err_t err;
	struct pbuf *p;
...
static void net_lwip_close_tcp(net_lwip_ctrl_struct* prvlwip, int socket_id)
{
...
//	uint8_t active_flag;
	...
	net_lwip_ctrl_struct* prvlwip = get_lwip_ctrl(adapter_index);
	if (prvlwip == NULL) {
		LLOGD("net_lwip_task adapter_index %d is NULL, fallback to STA", adapter_index);
		prvlwip = get_lwip_ctrl(NW_ADAPTER_INDEX_LWIP_WIFI_STA);
		if (prvlwip == NULL) {
			return;
		}
	}
...
#ifdef LUAT_USE_DNS
		net_lwip_dns_tx_next(prvlwip, &tx_msg_buf);
#endif
		if (!prvlwip->dns_client.is_run)
		{
			platform_stop_timer(prvlwip->dns_timer);
...
//		break;
	case EV_LWIP_SOCKET_CLOSE:
		...
		if (prvlwip->socket[socket_id].listen_tcp)
		{
			...
			if (prvlwip->socket[socket_id].pcb.tcp)
			{
				net_lwip_close_tcp(prvlwip, socket_id);
			}
...
static void net_lwip_check_network_ready(uint8_t adapter_index)
{
	...
	if (prvlwip == NULL)
		return;
	// LLOGD("CALL net_lwip_check_network_ready prvlwip %p", prvlwip);
	// LLOGD("CALL net_lwip_check_network_ready netif_network_ready %d", prvlwip->netif_network_ready);
	// LLOGD("CALL net_lwip_check_network_ready dns_client %p", &prvlwip->dns_client);
...
static uint32_t net_lwip_socket_read_data(net_lwip_ctrl_struct* prvlwip, int socket_id, uint8_t *buf, uint32_t *read_len, uint32_t len, socket_data_t *p)
{
	uint32_t dummy_len;
...
static int net_lwip_socket_receive(int socket_id, uint64_t tag,  uint8_t *buf, uint32_t len, int flags, luat_ip_addr_t *remote_ip, uint16_t *remote_port, void *user_data)
{
	...
	if (buf)
	{
		...
		if (prvlwip->socket[socket_id].is_tcp)
		{
			...
			{
				prvlwip->socket[socket_id].rx_wait_size -= net_lwip_socket_read_data(prvlwip, socket_id, buf + read_len, &read_len, len, p);
...
static int net_lwip_socket_send(int socket_id, uint64_t tag, const uint8_t *buf, uint32_t len, int flags, luat_ip_addr_t *remote_ip, uint16_t remote_port, void *user_data)
{
	...
	if (prvlwip->socket[socket_id].is_tcp)
	{
		while(save_len < len)
		{
			...
			p = net_lwip_create_data_node(prvlwip, socket_id, &buf[save_len], dummy_len, remote_ip, remote_port);
	...
	{
		p = net_lwip_create_data_node(prvlwip, socket_id, buf, len, remote_ip, remote_port);
		if (p)
		{
			llist_add_tail(&p->node, &prvlwip->socket[socket_id].tx_head);
		}
...
{
	net_lwip_ctrl_struct* prvlwip = get_lwip_ctrl(NW_ADAPTER_INDEX_LWIP_WIFI_STA);
	if (prvlwip == NULL)
		prvlwip = get_lwip_ctrl(NW_ADAPTER_INDEX_LWIP_WIFI_AP);
	if (prvlwip == NULL)
		return 0;
...
void net_lwip_set_netif(struct netif *netif, int adapter_index)
{
	...
	set_lwip_netif(netif, adapter_index);
	net_lwip_ctrl_struct* prvlwip = get_lwip_ctrl(adapter_index);
	if (prvlwip == NULL) {
		LLOGD("net_lwip_set_netif fail");
		return;
	}
	// LLOGD("net_lwip_set_netif is done");
	return;
}
